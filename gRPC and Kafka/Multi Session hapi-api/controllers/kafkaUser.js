...
const UserController = {
  ...
  logInUser: async (request, h) => {
    ...
    try {
      ...
      const loggedInUser = await userSession.findOne({
        email,
        deviceId,
        activeStatus: true,
      });
      ...
      if (loggedInUser) {
        return h
          .response({ message: "Already LoggedIn on this Device!!", token })
          .code(200);
      }
      ...
      async function sendOTP(phoneNumber) {
        ...
        await producer.send({
          topic: "otp-requests",
          messages: [{ value: JSON.stringify({phoneNumber}) }],
        });
        console.log(`OTP request sent for ${phoneNumber}`);
        await producer.disconnect();
      }

      // Call sendOTP instead of client.getOtp()
      sendOTP(phoneNumber);
    ...
    try {
      ...
      if (loggedInUser) {
        await userSession.updateOne(
          { email: email, deviceId: deviceId, activeStatus: true },
    ...
    return new Promise((resolve, reject) => {
      client.verifyOtp({ otp }, (error, response) => {
        if (error) {
          ...
          return resolve(
            h.response({ message: "OTP Verification Failed" }).code(500)
          );
        }

        if (response.verified) {
          resolve(
            h.response({ message: "OTP Verified Successfully" }).code(200)
          );
        } else {
